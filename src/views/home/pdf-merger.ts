...
export class PDFMerger {
  ...
  async add(inputFile: inputFile, pages?: string | string[] | undefined | null) {
    if (typeof pages === 'undefined' || pages === null) {
      ...
      return this._addFromToPage(inputFile, Number.parseInt(span[0]), Number.parseInt(span[1]))
  ...
  async _getInputFile(inputFile: inputFile) {
    if (inputFile instanceof ArrayBuffer) return inputFile
    ...
    if (inputFile instanceof window.Blob) return await inputFile.arrayBuffer()
